...
export class Router {
    ...
    public setRouter(address: string, abi: string, network: string, name: string, logo: string) {
    ...
    public async getByAddress(address: string): Promise<QuerySnapshot<DocumentData>> {
        return await getDocs(query(collection(db, "router"), where("address", "==", address)));
    }
    ...
    public async create(): Promise<any> {
        ...
        if(this.address !== undefined)
            ...
            if(nets.docs.length === 0)
                return await addDoc(await collection(db, "router"), { 
    ...
    public async all(): Promise<QuerySnapshot<DocumentData>> {
        return await getDocs(await collection(db, "router"));
    }
